...
export const Step3 = ({ setStep }: { setStep: (step: number) => void }) => {
  ...
  const [timeLeft, setTimeLeft] = useState(moment(endTime).diff(moment(currentTime), 'seconds'));

  const [isPaused, setIsPaused] = useState(false);  
  const [restStart, setRestStart] = useState(moment().toISOString());
  const [restEnd, setRestEnd] = useState(moment().toISOString());
  const [totalRest, setTotalRest] = useState(moment(restEnd).diff(moment(restStart), 'seconds'));
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      setProgress(prev =>
        Math.min(100, ((totalTime - (now.diff(moment(startTime), 'seconds'))) / totalTime) * 100)
      );
      ...
      if (isPaused) {
        ...
        setRestProgress(prev =>
          Math.min(100, ((totalRest - (now.diff(moment(restStart), 'seconds'))) / totalRest) * 100)
        );
        ...
        <div className="inline-flex flex-col justify-start items-center gap-20">
          <div data-time-left="paused" className="w-[472px] h-[472px] relative">
            ...
            {isPaused && (
              <div className="left-[130px] top-[146.50px] absolute inline-flex flex-col justify-center items-center gap-5">
                <div className="flex flex-col justify-start items-center gap-2.5">
                  <div className="justify-start text-[#c7c7c7] text-4xl font-normal font-['Istok_Web']">{formatMMSS(restLeft)} 남음</div>
                  <div className="justify-start text-white text-5xl font-normal font-['Inter']">일시정지됨</div>
                </div>
                <div className="self-stretch px-2.5 inline-flex justify-center items-center gap-5">
                  ...
                  <button
                    onClick={() => {
                      setRestEnd(moment(restEnd).add(5, 'minutes').toISOString());
                      setTotalRest(moment(restEnd).diff(moment(restStart), 'seconds'));
                      setRestLeft(moment(restEnd).diff(moment(currentTime), 'seconds'));
                      setRestProgress(0);
                    }}
                    className="w-10 h-10 relative overflow-hidden"
                  >
            ...
            {!isPaused && (
              <div className="h-[120px] left-[128px] top-[176.50px] absolute inline-flex flex-col justify-center items-center gap-5">
                <div className="justify-start text-white text-5xl font-normal font-['Inter']">{formatMMSS(timeLeft)} 남음</div>
                <div className="self-stretch px-2.5 inline-flex justify-center items-center gap-5">
                  <button
                    onClick={() => {
                      setIsPaused(true);
                      setRestStart(moment(currentTime).toISOString());
                      setRestEnd(moment(currentTime).add(10, 'minutes').toISOString());
                      setTotalRest(600);
                      setRestLeft(moment(restEnd).diff(moment(currentTime), 'seconds'));
                      setRestProgress(0);
                    }}
          ...
          <div className="w-[466px] h-20 bg-[#242121] rounded-[20px] inline-flex justify-start items-center overflow-hidden">
            <div className="flex-1 h-20 px-5 py-2.5 flex justify-center items-center gap-2.5">
              <div className="justify-start text-[#c7c7c7] text-2xl font-normal font-['Inter']">한 모금</div>
            ...
            <div className="flex-1 h-20 px-5 py-2.5 flex justify-center items-center gap-2.5">
              <div className="justify-start text-[#c7c7c7] text-2xl font-normal font-['Inter']">반샷</div>
              <div className="w-6 h-6 relative overflow-hidden">
                <div className="w-[18px] h-5 left-[3px] top-[2px] absolute bg-[#c7c7c7]"></div>
              </div>
            </div>
            <div className="w-px h-[45px] bg-[#685e5e]"></div>
            <div className="flex-1 h-20 px-5 py-2.5 flex justify-center items-center gap-2.5">
              <div className="justify-start text-[#c7c7c7] text-2xl font-normal font-['Inter']">원샷</div>
              <div className="w-6 h-6 relative overflow-hidden">
                <div className="w-[18px] h-5 left-[3px] top-[2px] absolute bg-[#c7c7c7]"></div>
              </div>
            </div>
